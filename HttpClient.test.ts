import { HttpClient, RequestConfig, Logger } from './HttpClient';
...
describe('HttpClient', () => {
  ...
  it('should make a GET request with query parameters', async () => {
    (global.fetch as jest.Mock).mockImplementation(() => mockResponse(200, 'OK', { key: 'value' }));
  ...
  it('should make a POST request with a JSON body', async () => {
    (global.fetch as jest.Mock).mockImplementation(() => mockResponse(200, 'OK', { success: true }));
  ...
  it('should handle errors correctly with default messages', async () => {
    (global.fetch as jest.Mock).mockImplementation(() => mockResponse(404, 'Not Found', { message: 'Not Found' }));
  ...
  it('should handle errors correctly with custom messages', async () => {
    ...
    await expect(apiClient.get('/endpoint')).rejects.toThrow('The resource was not found.');
  ...
  it('should call the logger when there is an error', async () => {
    ...
    (global.fetch as jest.Mock).mockImplementation(() =>
      mockResponse(500, 'Internal Server Error', { message: 'Internal Server Error' }),
    );
    ...
    await expect(apiClient.get('/endpoint')).rejects.toThrow('Internal server error occurred.');
